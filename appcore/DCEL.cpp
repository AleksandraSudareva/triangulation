...
//    }
    ...
    void DoubleEdgeList::initialize(std::initializer_list<Vector2> points)
    {
        int size = points.size();
...
//    }
    ...
    DoubleEdgeListHalfEdge* DoubleEdgeList::addVertex(DoubleEdgeListHalfEdge* edge, Vector2 point, int new_index){

        auto vertex = new DoubleEdgeListVertex(point);
        auto next = new DoubleEdgeListHalfEdge();
        auto next_twin = new DoubleEdgeListHalfEdge();

        next->origin = vertex;
        next_twin->origin = edge->getDestination();
        next->twin = next_twin;
        next_twin->twin = next;
        next->face = edge->face;
        next_twin->face = edge->twin->face;
        next->next = edge->next;
        next_twin->next = edge->twin;

        edge->twin->origin = vertex;
        edge->twin->getPrevious()->next = next_twin;
        edge->next = next;
    ...
    void DoubleEdgeList::addHalfEdges(DoubleEdgeListVertex* v1, DoubleEdgeListVertex* v2,
        ...
        auto adding_edge = new DoubleEdgeListHalfEdge();
        auto adding_edge_twin = new DoubleEdgeListHalfEdge();
        auto adding_face = new DoubleEdgeListFace();

        adding_edge->origin = v1;
        adding_edge_twin->origin = v2;
        adding_edge->face = adding_face;
        adding_edge_twin->face = e2->face; //e1->face
        adding_edge->twin = adding_edge_twin;
        adding_edge_twin->twin = adding_edge;
        adding_edge->next = e2->next;
        adding_edge_twin->next = e1->next;

        e1->next = adding_edge;
        e2->next = adding_edge_twin;
//        e1->getPrevious()->twin->next = adding_edge_twin;
//        e2->twin->next->next = adding_edge;

        adding_face->edge = e1;
        e1->face = adding_face;
        DoubleEdgeListHalfEdge* _edge = adding_edge;
        while (_edge != e1) {
            _edge->face = adding_face;
            _edge = _edge->next;
        }
        e2->face->edge = e2;

        faces.emplace_back(adding_face);
        edges.emplace_back(adding_edge);
        edges.emplace_back(adding_edge_twin);
    }
